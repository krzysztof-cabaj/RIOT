...
- The version of riot-wrappers that works both with the previous and the new code
    ...
    Updating the `Cargo.lock` is an automatic process done by `cargo update`.
    Since this has to be done for all Rust applications, you can use the
    following command to automate it for you, including a short build test
    to identify possible issues before merging.

    ```shell
    for x in examples/lang_support/official/rust* tests/rust*; do [ -f "$x/Cargo.lock" ] || continue; (cd "$x" && cargo update && make BOARD=samd21-xpro CARGO_CHANNEL=nightly) || break; done
